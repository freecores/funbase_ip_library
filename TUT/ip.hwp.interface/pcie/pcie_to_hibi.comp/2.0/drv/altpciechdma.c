...
#define DMA_TEST_TRANSFERS 1
#define DMA_RW_LENGTH 128
#define ENABLE_DMA_WRITE
#define ENABLE_DMA_READ
...
static inline void ape_chdma_desc_set(struct ape_chdma_desc *desc, dma_addr_t addr, u32 ep_addr, int len) {
  ...
  printk(KERN_DEBUG "desc->w0: %x\n", desc->w0);
  printk(KERN_DEBUG "desc->ep_addr: %x\n", desc->ep_addr);
  printk(KERN_DEBUG "desc->rc_addr_h: %x\n", desc->rc_addr_h);
  printk(KERN_DEBUG "desc->rc_addr_l: %x\n", desc->rc_addr_l);
}
...
static int __devinit dma_test(struct ape_dev *ape, struct pci_dev *dev) {
  int result = -1; // test result; guilty until proven innocent
  struct ape_chdma_header *write_header = (struct ape_chdma_header *)ape->bar[APE_BAR_HEADER]; // the DMA read header sits at address 0x00 of the DMA engine BAR
  struct ape_chdma_header *read_header = write_header + 1; // the write DMA header sits after the read header at address 0x10
  u8 *buffer_virt = 0; // virtual address of the allocated buffer
  dma_addr_t buffer_bus = 0; // bus address of the allocated buffer
  int i, n = 0, irq_count;
  
  u32 w; // temporary value used to construct 32-bit data words
  
  printk(KERN_DEBUG "\n");
  printk(KERN_DEBUG "--------- DMA TEST ---------\n");
  printk(KERN_DEBUG "\n");
  printk(KERN_DEBUG "bar_tests(), PAGE_SIZE = 0x%0x\n", (int)PAGE_SIZE);
  printk(KERN_DEBUG "fpga: read dma registers  = 0x%p.\n", write_header);
  printk(KERN_DEBUG "fpga: write dma registers = 0x%p.\n", read_header);
  ...
  // allocate and map coherently-cached memory for a DMA-able buffer @see Documentation/PCI/PCI-DMA-mapping.txt, near line 318
  ...
  for (i = 0; i < 4 * PAGE_SIZE; i += 4) { // fill first half of buffer with its virtual address as data
    *(u32 *)(buffer_virt + i) = (u32)(unsigned long)(0xA0A00000 + i); //(buffer_virt + i);
  }
  
  ape->table_virt->w3 = cpu_to_le32(0x0000FADE); // invalidate EPLAST, outside 0-255, 0xFADE is from the testbench
  
  n = 0; //fill in first descriptor
  
  // read 8192 bytes from RC buffer to EP address 4096
//  ape_chdma_desc_set(&ape->table_virt->desc[n], buffer_bus, 4096, DMA_RW_LENGTH); //2 * PAGE_SIZE);
  for (i = 0; i < DMA_TEST_TRANSFERS; i++) {
    printk(KERN_DEBUG "creating desc %d:\n", i);
    ape_chdma_desc_set(&ape->table_virt->desc[i], buffer_bus, 4096, DMA_RW_LENGTH); // 2 * PAGE_SIZE);
  }
  
  n = i;
  
//  if (ape->msi_enabled) { // enable MSI after the last descriptor is completed
...
//  }
  ...
  for (i = 0; i < n; i++) {
    ...
    p += i*4;
    printk(KERN_DEBUG "desc %d:\n", i);
    printk(KERN_DEBUG "0x%08x: 0x%08x (LEN=0x%x)\n", (u32)p, *p, 4 * le32_to_cpu(*p));
    p++;
    printk(KERN_DEBUG "0x%08x: 0x%08x (EPA=0x%x)\n", (u32)p, *p, le32_to_cpu(*p));
    p++;
    printk(KERN_DEBUG "0x%08x: 0x%08x (RCH=0x%x)\n", (u32)p, *p, le32_to_cpu(*p));
    p++;
    printk(KERN_DEBUG "0x%08x: 0x%08x (RCL=0x%x)\n", (u32)p, *p, le32_to_cpu(*p));
    printk(KERN_DEBUG "\n");
  }
  
  w = (u32)(n); // set available number of descriptors in table
...
//  }
  ...
  printk(KERN_DEBUG "read 0x%08x from 0x%p\n", ioread32(&read_header->bdt_addr_l), (void *)&read_header->bdt_addr_l);
  
  wmb(); // memory write barrier

//  printk(KERN_DEBUG "Flush posted writes\n");
//  (void)ioread32(); // FIXME Add dummy read to flush posted writes but need a readable location!
  
  irq_count = ape->irq_count; // remember IRQ count before the transfer
  
  #ifdef ENABLE_DMA_WRITE
  printk(KERN_DEBUG "\nStart DMA write\n");
  printk(KERN_DEBUG "\n");
  iowrite32(n-1, &read_header->w3); // write last desc number - this starts the DMA
  #endif
  
  wmb(); // memory write barrier
  
//  (void)ioread32(); // dummy read to flush posted writes, FIXME Need a readable location!
...
//       printk(KERN_DEBUG "#IRQs during transfer: %d\n", ape->irq_count - irq_count); // print IRQ count before the transfer
//       break;
//     }
//     udelay(100);
//   }
  ...
  n = 0;
//  ape_chdma_desc_set(&ape->table_virt->desc[n], buffer_bus + 8192, 4096, DMA_RW_LENGTH); //2 * PAGE_SIZE);
  for (i = 0; i < DMA_TEST_TRANSFERS; i++) {
    printk(KERN_DEBUG "creating desc %d:\n", i);
    ape_chdma_desc_set(&ape->table_virt->desc[i], buffer_bus + 8192, 4096, DMA_RW_LENGTH); //2 * PAGE_SIZE);
  }
  
  n = i;

  // test variable, make a module option later
//  if (ape->msi_enabled) {
//    ape->table_virt->desc[n].w0 |= cpu_to_le32(1UL << 16); //local MSI
//  }
  ...
  w = (u32)n; // set number of available descriptors in the table
...
//  }
  ...
  wmb(); // memory write barrier and flush posted writes
  
//  (void)ioread32(); // dummy read to flush posted writes, FIXME Need a readable location!
  
  irq_count = ape->irq_count;
  
  #ifdef ENABLE_DMA_READ
  printk(KERN_DEBUG "\nStart DMA read\n");
  iowrite32(n-1, &write_header->w3);
  #endif
  
  wmb(); // memory write barrier
  
//  (void) ioread32(); // dummy read to flush posted writes
...
//  iowrite32(0x0000ffffUL, &write_header->w0); // soft-reset DMA write engine
//  iowrite32(0x0000ffffUL, &read_header->w0); // soft-reset DMA read engine
...
//  (void)ioread32(); // dummy read to flush posted writes, FIXME Need a readable location!
  ...
  udelay(10000);
  printk(KERN_DEBUG "\n");
  printk(KERN_DEBUG "read result:\n");
  i = 0;
  while (i < DMA_RW_LENGTH) {
    printk(KERN_DEBUG "0x%08x: %08x\n", buffer_bus + i + 8192, (u32*)(buffer_bus + i + 8192));
    i += 4;
  }
  printk(KERN_DEBUG "\n");
  #endif
...
//  result = compare((u32 *)buffer_virt, (u32 *)(buffer_virt + 128), 8192);
...
static void __exit alterapciechdma_exit(void) {
  ...
  pci_unregister_driver(&pci_driver); // unregister this driver from the PCI bus driver
}
